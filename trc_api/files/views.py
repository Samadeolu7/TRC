...
class SermonAudio(Resource):
            ...
            def get(self, id):
                sermon = Sermons.query.get(id)
                audio_path = os.path.join(current_app.root_path, sermon.audio_file)
                if audio_path:
                    if os.path.exists(audio_path) and os.access(audio_path, os.R_OK):
                        return send_file(audio_path)
                    else:
                        return {'message': 'The audio file cannot be accessed'}, 500
                return {'message': 'The sermon does not have an audio file'}, 404